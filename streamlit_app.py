...
try:
    # 1. ดึงข้อมูลแบบดิบที่สุด
    df = pd.read_csv(URL_FORM)
    
    # 2. ทำความสะอาดชื่อคอลัมน์: ตัดช่องว่าง และตัดพวก _x, _y ทิ้งให้หมด
    df.columns = df.columns.str.strip().str.replace(r'_[xy]$', '', regex=True)
    
    # 3. ลบคอลัมน์ที่ชื่อซ้ำกันออกไปเลย (เหลือไว้แค่ชื่อละ 1)
