...
# ดึงข้อมูลแยกชีตเด็ดขาด ไม่ใช้การ Merge
...
try:
    # 1. ดึงข้อมูล
    df_log = get_clean_data("การตอบแบบฟอร์ม 1")
    df_master = get_clean_data("Master_Tests")
    ...
    # 4. ดึงค่า Mean/SD จาก Master ตรงๆ (ป้องกันค่าผิดพลาด)
    master_info = df_master[df_master['รายการทดสอบ'] == selected_test].iloc[0]
    
    # 5. กราฟ LJ พร้อมเส้น SD ตามมาตรฐาน Westgard
    def draw_lj(data, mean, sd, col, title):
        ...
        for i in [1, 2, 3]:
            ...
            fig.add_hline(y=mean-(i*sd), line_dash="dash", line_color=colors[i])
        
        fig.update_layout(title=title, template="plotly_white")
        return fig

    # แสดงกราฟ Level 1 และ 2
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(draw_lj(display_df, master_info['L1_Mean'], master_info['L1_SD'], 'ผล Level 1', 'Level 1'), use_container_width=True)
    with c2:
        st.plotly_chart(draw_lj(display_df, master_info['L2_Mean'], master_info['L2_SD'], 'ผล Level 2', 'Level 2'), use_container_width=True)
